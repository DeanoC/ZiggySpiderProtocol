...
pub const NodeService = struct {
    pub const NamespaceFileUpdate = fs_node_ops.NamespaceFileUpdate;
    ...
    pub fn upsertNamespaceFilesWithEvents(
        self: *NodeService,
        updates: []const NamespaceFileUpdate,
    ) ![]fs_protocol.InvalidationEvent {
        self.mutex.lock();
        defer self.mutex.unlock();
        self.ops.clearPendingEvents();
        for (updates) |update| try self.ops.upsertNamespaceFile(update);
        return self.ops.copyPendingEvents(self.allocator);
    }
    ...
    pub fn readNamespaceFileContent(
        ...
        self.mutex.lock();
        defer self.mutex.unlock();
        return self.ops.readNamespaceFileContent(allocator, source_id, path);
    }
    ...
    fn maybeHandleChatInputWrite(
        ...
        try self.maybeUpdateChatCompanionVenoms(submission.job_id, effective_correlation, input, &submission);
    ...
    fn maybeUpdateChatCompanionVenoms(
        self: *NodeService,
        job_id: []const u8,
        correlation_id: []const u8,
        input: []const u8,
        submission: *const ChatInputSubmission,
    ) !void {
        const latest_thought = if (submission.result_text) |value|
            value
        else
            input;

        if (self.ops.readNamespaceFileContent(self.allocator, "thoughts", "history.ndjson") catch null) |existing_history| {
            defer self.allocator.free(existing_history);
            const history_line = try buildThoughtHistoryLine(
                self.allocator,
                latest_thought,
            );
            defer self.allocator.free(history_line);
            const merged_history = if (existing_history.len == 0)
                try self.allocator.dupe(u8, history_line)
            else
                try std.fmt.allocPrint(self.allocator, "{s}{s}", .{ existing_history, history_line });
            defer self.allocator.free(merged_history);

            const thoughts_status = try buildThoughtStatusJson(self.allocator, latest_thought);
            defer self.allocator.free(thoughts_status);
            const thought_updates = [_]NamespaceFileUpdate{
                .{ .source_id = "thoughts", .path = "latest.txt", .content = latest_thought, .writable = false },
                .{ .source_id = "thoughts", .path = "history.ndjson", .content = merged_history, .writable = false },
                .{ .source_id = "thoughts", .path = "status.json", .content = thoughts_status, .writable = false },
            };
            for (thought_updates) |update| {
                self.ops.upsertNamespaceFile(update) catch |err| switch (err) {
                    error.FileNotFound => {},
                    else => return err,
                };
            }
        ...
        const event_json = try buildChatEventNextJson(self.allocator, job_id, correlation_id, submission.state);
        defer self.allocator.free(event_json);
        const event_updates = [_]NamespaceFileUpdate{
            .{ .source_id = "events", .path = "next.json", .content = event_json, .writable = false },
            .{ .source_id = "events", .path = "STATUS.json", .content = event_json, .writable = false },
        };
        for (event_updates) |update| {
            self.ops.upsertNamespaceFile(update) catch |err| switch (err) {
                error.FileNotFound => {},
                else => return err,
            };
...
fn buildThoughtHistoryLine(allocator: std.mem.Allocator, content: []const u8) ![]u8 {
    const escaped_content = try fs_protocol.jsonEscape(allocator, content);
    defer allocator.free(escaped_content);
    return std.fmt.allocPrint(
        allocator,
        "{{\"seq\":{d},\"ts_ms\":{d},\"source\":\"text\",\"round\":1,\"content\":\"{s}\"}}\n",
        .{ std.time.milliTimestamp(), std.time.milliTimestamp(), escaped_content },
    );
}

fn buildThoughtStatusJson(allocator: std.mem.Allocator, latest: []const u8) ![]u8 {
    const escaped_latest = try fs_protocol.jsonEscape(allocator, latest);
    defer allocator.free(escaped_latest);
    return std.fmt.allocPrint(
        allocator,
        "{{\"count\":1,\"updated_at_ms\":{d},\"latest_source\":\"text\",\"latest_round\":1,\"latest\":\"{s}\"}}",
        .{ std.time.milliTimestamp(), escaped_latest },
    );
}

fn buildChatEventNextJson(
    allocator: std.mem.Allocator,
    job_id: []const u8,
    correlation_id: []const u8,
    state: fs_node_ops.ChatJobState,
) ![]u8 {
    const escaped_job_id = try fs_protocol.jsonEscape(allocator, job_id);
    defer allocator.free(escaped_job_id);
    const escaped_correlation = try fs_protocol.jsonEscape(allocator, correlation_id);
    defer allocator.free(escaped_correlation);
    return std.fmt.allocPrint(
        allocator,
        "{{\"event_id\":{d},\"source_path\":\"/jobs/{s}/status.json\",\"event_path\":\"/jobs/{s}/status.json\",\"updated_at_ms\":{d},\"correlation_id\":\"{s}\",\"state\":\"{s}\"}}",
        .{
